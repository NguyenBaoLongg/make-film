...
{{
  "project": {{
    ...
    "film_bible": {{
      ...
      "theme": "",
      "tone": "",
      "visual_style": "{settings.get('style', '')}",
      "color_palette": []
    }}
  }}
}}
...
{idea}"""
        ...
        p2 = """Using this Film Bible, create only the Character Library.
...
Return valid JSON only: { "characters": [ ... ] }"""
        ...
        p3 = """Using this Film Bible, create only the Location Library.
...
Each shot must include: id, duration_seconds, reference_ids (list of char/loc ids), image_prompt, video_prompt.
Every video_prompt must begin: "Use the provided image as the exact mandatory first frame."
Return valid JSON only: {{ "id": "{scene_id}", "shots": [ ... ] }}"""
            shots_res = send_and_get_json(page, p5)
            # Dữ liệu trả về có thể bọc trong dict có "shots" hoặc là "id" và "shots"
            # Cần đảm bảo có đủ data
            scene_final = {
                "id": scene_id,
